...
def take_input(prompt: str) -> str:
    """
    Prompts the user for input based on the given message from the LLM.

    Args:
        prompt (str): A prompt string generated by the LLM requesting input.
...
def expand_query(
    ...
    Args:
        state (AgentState): Contains:
            ...
            - state["user_data"]: Any previously stored query data
...
@tool
def answer_query(state: AgentState) -> None:
    """
...
def input_query_Agent(state: AgentState) -> AgentState:
    """
    ...
    final_prompt = SystemMessage(content=(
        ...
        "Your goal is to take user input and generate a contextually clear query that will be answered by another LLM. "
        "You should call the tool `take_input` if any of the following conditions are met: "
        "1. There is no conversation history. "
        "2. The user's query is very unclear and do not result in any meaningful query for context retrieval."
        "3. The query has already been answered — in this case, collect feedback. If the user is not satisfied, ask for the reason or extra information to optimise query; "
        "4. If user say quit/exit/close/bye as desire to exit the program: You must return 'exiting the program' as reponse"
        "if they are satisfied, prompt them to ask a follow-up question. "
        "After gathering all required information, return the final query in Following format:\n"
        "Query: actual query here"
        f"Here is the previous conversational context: {get_context(state)}"
    ))
    llm_response = llm_query_input.invoke([final_prompt])

    append_to_response([{"input_query": llm_response}], filename="check_agent_log.json")
...
llm_query_input = ChatGroq(model=MODEL_NAME).bind_tools(
    [take_input]
)
...
graph.add_node('Input_Agent', input_query_Agent)
graph.add_node('Take_input_ToolNode', ToolNode([take_input]))
...
def route_user(state: AgentState) -> str:
    ...
    if calls:
        ...
        if tool_name == "take_input":
            return "Take_input_ToolNode"
...
graph.add_conditional_edges('Input_Agent', route_user, {
    "END":END,
    "Take_input_ToolNode":"Take_input_ToolNode",
    "Query_Redirection_Agent":"Query_Redirection_Agent"

})

graph.add_edge("Take_input_ToolNode","Input_Agent")
...
graph.add_edge("Answer_Query_ToolNode","Input_Agent")
